function [ cum_wealth, daily_incre_fact, daily_port_total] = AICTR_run( data,win_size,tran_cost)
%{
...
for t = 1:T
    ...
    daily_port_o = daily_port.*data(t, :)'/(data(t, :)*daily_port);
     ...
     if(t<T)
       ...
       [daily_port_n,datahat,datahat_center,EMA]=AICTR(close_price,data,t+1,daily_port,datahat,datahat_center,EMA, win_size);
